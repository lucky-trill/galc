...
- If you only need the level change function of this module, I highly recommend Dewi's [https://foundryvtt.com/packages/elevation-level-change](Elevation Level Change) as a potential alternative. The two modules use different methods for determining when and where to make level changes, so depending on your needs and likes, you may find one or the other suits you better if you don't need GALC's additional level-related options like absolute elevation display. Because GALC and ELC are trying to do the same task, ***it is not recommended you use both at once***. Experiment with the two at your leisure and decide which you want to use.
